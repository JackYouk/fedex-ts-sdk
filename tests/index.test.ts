...
import { APIPromise } from 'Fedex/core/api-promise';

import util from 'node:util';
import Fedex from 'Fedex';
import { APIUserAbortError } from 'Fedex';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['FEDEX_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Fedex) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Fedex({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new Fedex({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Fedex({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['FEDEX_LOG'] = 'debug';
      const client = new Fedex({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['FEDEX_LOG'] = 'not a log level';
      const client = new Fedex({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'FEDEX_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['FEDEX_LOG'] = 'debug';
      const client = new Fedex({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  test('normalized method', async () => {
    ...
    const client = new Fedex({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Fedex({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['FEDEX_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Fedex({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['FEDEX_BASE_URL'] = 'https://example.com/from_env';
      const client = new Fedex({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['FEDEX_BASE_URL'] = ''; // empty
      const client = new Fedex({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://apis-sandbox.fedex.com');
    });

    test('blank env variable', () => {
      process.env['FEDEX_BASE_URL'] = '  '; // blank
      const client = new Fedex({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://apis-sandbox.fedex.com');
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new Fedex({ apiKey: 'My API Key', environment: 'production' }),
      ).toThrowErrorMatchingInlineSnapshot(
        `"Ambiguous URL; The \`baseURL\` option (or FEDEX_BASE_URL env var) and the \`environment\` option are given. If you want to use the environment you must pass baseURL: null"`,
      );

      const client = new Fedex({ apiKey: 'My API Key', baseURL: null, environment: 'production' });
      expect(client.baseURL).toEqual('https://apis-sandbox.fedex.com');
    });
  });

  test('maxRetries option is correctly set', () => {
    const client = new Fedex({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Fedex({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['FEDEX_API_KEY'] = 'My API Key';
    const client = new Fedex();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['FEDEX_API_KEY'] = 'another My API Key';
    const client = new Fedex({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Fedex({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Fedex({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Fedex({ apiKey: 'My API Key', fetch: testFetch });
